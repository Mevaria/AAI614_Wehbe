...
def _add_markers(figure_data, molecules, plot_type="scatter3d"):
    ...
    # Ensure positional alignment by converting potential pandas Series/np arrays to plain lists
    list_of_drugs = list(drug_data.get("text", []))
    x_vals = list(drug_data.get("x", []))
    y_vals = list(drug_data.get("y", []))
    # Only needed for 3D scatter
    z_vals = list(drug_data.get("z", [])) if plot_type == "scatter3d" else None
    ...
    for point_number in indices:
        # Guard against out-of-range issues just in case
        if point_number >= len(x_vals) or point_number >= len(y_vals):
            continue
        trace = {
            "x": [x_vals[point_number]],
            "y": [y_vals[point_number]],
            "mode": "markers",
            "marker": {"color": "red", "size": 16, "opacity": 0.6, "symbol": "cross"},
            "type": plot_type,
            "text": [list_of_drugs[point_number]] if point_number < len(list_of_drugs) else None,
        }
        if plot_type == "scatter3d" and z_vals is not None and point_number < len(z_vals):
            trace["z"] = [z_vals[point_number]]
        traces.append(trace)
    return traces
